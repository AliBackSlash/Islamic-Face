...
public class FileService(IWebHostEnvironment _env, IHttpContextAccessor _httpContextAccessor,IOptionsMonitor<StaticFilesSettings> _settings) : IFileService
{
    ...
    public async Task<Result<UploadResponse>> UploadProfileCoverAsync(IFormFile file)
    {
        var saveResult =  await SaveFileAsync(file, _settings.CurrentValue.ProfileCovers_FileName, _settings.CurrentValue.MaxProfileCoverSize);
    ...
    public async Task<Result<List<string>>?> UploadPostImagesAsync(List<IFormFile> files)
    {
        ...
        {
            var url = await SaveFileAsync(file, _settings.CurrentValue.PostImages_FileName, _settings.CurrentValue.MaxPostImageSize);
            if (url != null)
                urls.Add(url.Value);
        }
