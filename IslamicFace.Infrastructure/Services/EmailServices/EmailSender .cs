...
namespace IslamicFace.Infrastructure.Services.EmailServices;
...
public class EmailService(IConfiguration _config) : IEmailService
{
    ...
    public async Task<Result<int>> SendOTPCodeAsync(string To, CancellationToken cancellationToken, string Subject = "IslamicFace – Your Password Reset Code")
    {
    ...
    public async Task<Result<int>> SendRestPasswordTokenAsync(string To,string token, CancellationToken cancellationToken, string Subject = "IslamicFace – Reset Your Password")
    {
        int OTPCode = Random.Shared.Next(100000, 999999);
